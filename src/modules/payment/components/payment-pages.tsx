...
const money=(value:number|undefined,lang:string)=>new Intl.NumberFormat(lang==="vi"?"vi-VN":"en-US",{style:"currency",currency:"VND",maximumFractionDigits:0}).format(value??0);
export function PaymentsPage({scope}:{scope:"customer"|"pt"}){
 const {t,i18n}=useTranslation();const [status,setStatus]=useState<PaymentStatus|"">("");const [creating,setCreating]=useState(false);const [refunding,setRefunding]=useState<Payment|null>(null);const query=usePayments(scope,status||undefined);const items=query.data?.content??[];
 const total=items.reduce((sum,p)=>sum+(scope==="pt"?(p.ptEarning??0):(p.amount??0)),0);
 return <div><section className="mb-6 flex flex-col gap-5 rounded-3xl border border-zinc-200 bg-white/80 p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/80 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-3 h-1 w-10 rounded-full bg-orange-500"/><h1 className="text-3xl font-black">{t(`payment.${scope}Title`)}</h1><p className="mt-2 text-sm text-zinc-500">{t(`payment.${scope}Description`)}</p></div>{scope==="customer"&&<Button onClick={()=>setCreating(true)}><Plus className="size-4"/>{t("payment.create")}</Button>}</section>
 {!!items.length&&<div className="mb-5 flex items-center gap-4 rounded-2xl border border-zinc-200 bg-white p-5 dark:border-zinc-800 dark:bg-zinc-950"><WalletCards className="size-7 text-lime-600"/><div><p className="text-xs font-black uppercase text-zinc-400">{t(scope==="pt"?"payment.totalEarning":"payment.totalAmount")}</p><p className="text-2xl font-black">{money(total,i18n.language)}</p></div></div>}
 <select className={selectClassName} value={status} onChange={e=>setStatus(e.target.value as PaymentStatus|"")}><option value="">{t("payment.allStatuses")}</option>{statuses.map(s=><option key={s} value={s}>{t(`payment.statuses.${s}`)}</option>)}</select>
