...
function SystemConfigs() {
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      setEditing(null);
      toast({ type: "success", title: "Đã cập nhật tham số" });
    },
    onError: (e) => toast({ type: "error", title: "Lỗi", description: toErrorMessage(e) }),
  });
    ...
    <div className="bg-card rounded-xl border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        <h2 className="flex items-center gap-2 text-sm font-semibold text-foreground">
          <SlidersHorizontal className="size-4 text-primary" /> Tham số hệ thống
        </h2>
      </div>
      {isLoading ? (
        <div className="flex items-center justify-center py-8"><Loader2 className="size-5 animate-spin text-muted-foreground" /></div>
      ) : configs.length === 0 ? (
        <p className="text-xs text-muted-foreground py-4 text-center">Chưa có tham số.</p>
      ) : (
        <div className="overflow-x-auto"><table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide border-b border-border">
          ...
          <tbody className="divide-y divide-border">
            {configs.map((c) => (
              <tr key={c.key}>
                <td className="py-2.5 font-mono text-xs font-semibold text-foreground">{c.key}</td>
                <td className="py-2.5 font-bold text-foreground">{c.value}</td>
                <td className="py-2.5 text-xs text-muted-foreground">{c.description || "—"}</td>
                <td className="py-2.5 text-right">
                  <button
                    onClick={() => { setEditing(c); setValue(c.value ?? ""); }}
      ...
      <Dialog open={!!editing} title={`Cập nhật ${editing?.key ?? ""}`} onClose={() => setEditing(null)}>
        <div className="space-y-4">
          {editing?.description && <p className="text-xs text-muted-foreground">{editing.description}</p>}
          <div>
            <label className="block text-xs font-semibold text-muted-foreground mb-1.5">Giá trị (số nguyên dương)</label>
            <Input type="number" min={1} value={value} onChange={(e) => setValue(e.target.value)} />
          </div>
          <div className="flex justify-end gap-2">
            <Button onClick={() => setEditing(null)} className="bg-card border border-border text-muted-foreground hover:bg-muted/40 shadow-none">Hủy</Button>
            <Button onClick={() => save.mutate()} disabled={save.isPending} className="gap-2 bg-primary hover:bg-primary/90 text-white">
              {save.isPending && <Loader2 className="size-4 animate-spin" />} Lưu
            </Button>
          </div>
        </div>
      </Dialog>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">Danh mục dịch vụ dùng chung và tham số vận hành của nền tảng.</p>
      </div>
      <ServiceCategories />
      <SystemConfigs />
    </div>
