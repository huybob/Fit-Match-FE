...
import { Heart, UserRound, MapPin, Building2 } from "lucide-react";
...
function FavoriteGymsSection() {
  ...
  return (
    <div className="mt-10">
      ...
      <p className="mb-6 text-sm text-muted-foreground">Các phòng tập bạn đã lưu để đặt lịch nhanh.</p>
      ...
      {query.isLoading ? (
        ...
        <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
          {items.map((gym) => (
            <article key={gym.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
    ...
    <div className="min-h-screen bg-muted/40">
      <div className="flex gap-6 px-20 py-6">
        ...
        <main className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted-foreground mb-6">Danh sách PT bạn đã lưu để xem lại và liên hệ sau.</p>
          ...
          {query.isLoading ? (
            ...
            <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-3">
              {items.map((pt) => (
                <article key={pt.id} className="rounded-2xl border border-border bg-card p-5 shadow-sm">
                  <div className="flex items-center gap-3">
